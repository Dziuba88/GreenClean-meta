...
for (i = 0; i < x.length; i++) {
  ...
  b.setAttribute("class", "selectbox--items selectbox--hide");
    ...
    c.addEventListener("click", function(e) {
        ...
        for (i = 0; i < s.length; i++) {
          if (s.options[i].innerHTML == this.innerHTML) {
            ...
            y = this.parentNode.getElementsByClassName("current");
...
function closeAllSelect(elmnt) {
  ...
  y = document.getElementsByClassName("selectbox");
  ...
  for (i = 0; i < x.length; i++) {
    if (arrNo.indexOf(i)) {
      x[i].classList.add("selectbox--hide");
    }
  }
}
